...
function getByFoodId($table, $id)
{
	$query = "SELECT * FROM " . $table . " WHERE food_id=" . $id . " ";
	$result = qSELECT($query);
	if ($result)
		return $result[0];
	else
...
function getByExerId($table, $id)
{
	$query = "SELECT * FROM " . $table . " WHERE exercise_id=" . $id . " ";
	$result = qSELECT($query);
	if ($result)
		return $result[0];
	else
